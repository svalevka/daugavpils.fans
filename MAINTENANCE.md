...
регистрация или git. Есть два способа, на выбор:

**Вариант 1. Через сайт, самостоятельно, сразу.** На странице самой
группы или альбома есть кнопка **«Предложить изменение»** (ведёт на
...
фото/видео со своего телефона или компьютера. Предложение проверит один
из кураторов проекта. С текстовыми исправлениями после одобрения всё
происходит автоматически — они сразу появляются на сайте. С фото и видео
чуть иначе: после одобрения куратору ещё нужно вручную опубликовать файл
в архиве, так что это может занять чуть больше времени. Это самый
простой способ — подходит всем.

Эта возможность есть только на основном сайте daugavpils.fans — на
резервной копии на GitHub Pages (см. ниже) такой формы нет.

**Вариант 2. Через pull request на GitHub, если вы разбираетесь в git.**
Можно отредактировать нужный файл (`band.yaml`/`release.yaml` в папке
группы или альбома в репозитории) напрямую и открыть pull request с
изменением. Такой pull request проверяют кураторы проекта, и после
одобрения изменение попадает в архив. Этот способ чуть более технический
(понадобится аккаунт на GitHub и знакомство с git), но учтите: через
pull request можно менять только текст (`band.yaml`/`release.yaml`) —
сами файлы фото/видео/записей через GitHub не передаются (они туда
специально не попадают) и публикуются отдельно куратором, у которого
есть доступ к archive.org. Если нужно именно добавить новое фото или
видео — используйте вариант 1 выше; точный технический процесс для
текстовых pull request'ов описан в [README.md](README.md).
